...
public class ARBasketballGameControl : MonoBehaviour
{
	[Header("UI")]
	[SerializeField]
	...
	[SerializeField]
	private ThrowControl throwControlForPreparing;

	[SerializeField]
	private GameObject throwingPrefabForARFoundation;
	...
	private IEnumerator InitThrowingCoroutine(Camera camera)
	{
		if (camera)
		{
			throwControlForPreparing.cameraMain = camera;
			throwControlForPreparing.SetPrefabBeforeInit(
				throwingPrefabForARFoundation);
		}
